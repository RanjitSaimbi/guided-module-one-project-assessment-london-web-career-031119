...
class Battle
  ...
  def round_2
    ...
    if @user_pokemon.send(:"#{tr_choice}") > @team_rocket_pokemon.send(:"#{tr_choice}")
      puts_medium PASTEL.white("Your #{@user_pokemon.name} inflicted ") + PASTEL.bright_white.on_bright_red.bold("#{@user_pokemon.send(:"#{tr_choice}")} DAMAGE!")
      puts ""
      ...
      puts_medium PASTEL.white("Team Rocket's #{@team_rocket_pokemon.name} used #{tr_choice} and inflicted ") + PASTEL.bright_white.on_bright_red.bold("#{@team_rocket_pokemon.send(:"#{tr_choice}")} DAMAGE!")
      puts ""
